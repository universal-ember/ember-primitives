...
**Replaced by CSS Anchor Positioning.** All higher-level components (`<Popover>`, `<Menu>`, etc.) now use native [CSS Anchor Positioning](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_anchor_positioning) instead of Floating UI. The `<FloatingUI>` component and `anchorTo` modifier remain available for advanced use cases where you need direct access to [Floating UI](https://floating-ui.com/) middleware, but for most needs, prefer [`<Popover>`](/5-floaty-bits/popover.md) or [`<Menu>`](/5-floaty-bits/menu.md) directly.
