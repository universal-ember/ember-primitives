  ...
  module('preferred usage', function () {
    ...
    test('can be closed', async function (assert) {
      await render(<template>
        <Modal as |m|>
          <out>{{m.isOpen}}</out>
          <button id='open' {{on 'click' m.open}}>Open Dialog</button>
          <m.Dialog>
            content
            <button id='close' {{on 'click' m.close}}>Close Dialog</button>
          </m.Dialog>
        </Modal>
      </template>);
      ...
      await render(<template>
        <Modal @onClose={{handleClose}} as |m|>
          ...
          <m.Dialog>
            content
            <form method='dialog'>
              <button {{on 'click' m.close}} value='resetBtn'>Reset</button>
